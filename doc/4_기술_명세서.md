...
### 3.4 Performance & SEO
-   **Caching Strategy**:
    -   `firebase.json`을 통해 정적 자산(JS/CSS/Image)에 `Cache-Control: public, max-age=31536000, immutable` 헤더 적용.
    -   HTML 및 Service Worker는 `no-cache`로 설정하여 배포 즉시 업데이트 반영 보장.
-   **Bundling Optimization**:
    -   `vite.config.js`에서 `react-icons` 등 대형 라이브러리의 불필요한 번들링을 방지하고 Tree Shaking 효율화.
-   **SEO & Web Standards**:
    -   `robots.txt` 설정: 모든 크롤러 접근 허용(`Allow: /`).
    -   `index.html`: Meta Description, Keywords, Open Graph(OG) 태그 적용으로 검색 및 공유 최적화.
