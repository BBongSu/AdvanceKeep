...
-   **Window File System Access API**:
    ...
    -   **Native File Export**:
        -   **Tech**: `@capacitor/filesystem`, `@capacitor/share`
        -   **Flow**: JSON 파일 생성(Cache Directory) -> `Share.share()` 호출 -> OS 네이티브 공유 시트 표시.
        -   **Benefit**: 안드로이드 10+ 범위 지정 저장소(Scoped Storage) 문제 해결 및 직관적인 UX.

### 3.3 Mobile Application (Native Container)
-   **Capacitor**: v7.0.0
    -   **Cross-Platform**: 단일 웹 코드베이스로 Android 및 iOS 네이티브 앱 생성.
    -   **Web Native Bridge**: 웹 앱을 네이티브 웹뷰에서 실행하며, 필요 시 네이티브 기능 호출.
    -   **Configuration**: `capacitor.config.ts` 및 `android/`, `ios/` 네이티브 프로젝트 폴더 관리.
    -   **Plugins**:
        -   `@codetrix-studio/capacitor-google-auth`: 네이티브 구글 로그인 지원 (웹 팝업 차단 문제 해결).
        -   `@capacitor/share`: 파일 공유 및 백업 저장.
        -   `@capacitor/filesystem`: 로컬 파일 임시 생성.

### 3.4 Data Backup & Restore Logic
